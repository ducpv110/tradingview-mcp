...
const set = (await c.Runtime.evaluate({
    expression: `(function() {
        ...
        for (var i = 0; i < 20; i++) {
            ...
            fk = Object.keys(el).find(function(k) {
...
if (!set) {
    console.error('Could not inject into Pine editor');
    await c.close();
    process.exit(1);
}
...
const clicked = (await c.Runtime.evaluate({
    expression: `(function() {
        ...
        for (var i = 0; i < btns.length; i++) {
            ...
            if (/save and add to chart/i.test(t)) {
                btns[i].click();

                return t;
            }
            
            if (/^(Add to chart|Update on chart)/i.test(t)) {
                btns[i].click();
                ...
                if (env.editor && typeof env.editor.getEditors === "function") {
                    ...
                    if (eds.length > 0) {
                        ...
                        return markers.map(function(m) {
                            return { line: m.startLineNumber, msg: m.message };
                        });
                    }
                }
            }
