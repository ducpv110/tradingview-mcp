...
let client = null;
let targetInfo = null;
// Override via TV_CDP_HOST/TV_CDP_PORT (or CDP_HOST/CDP_PORT) env vars.
...
export async function connect(targetId = null) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            if (!target) {
                throw new Error(targetId ? `CDP target ${targetId} not found - is the tab still open?` : 'No TradingView chart target found. Is TradingView open with a chart?');
            }
